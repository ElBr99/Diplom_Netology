...
- В докере запустятся 3 приложения:
  - backend-server, Java 11 на порту: ```http://localhost:8181```;
  - frontend-client, Node 15 на порту: ```http://localhost:8080```;
  - database-server на порту: ```http://localhost:5432```;
- Запустить тесты можно:
  - Через терминал командой `mvn test`;

### Для тестирования frontend + backend + postgresql необходимо создать пользователей в postgresql
